...
# ---- Optional: Upweight Q residuals when Q_true > 100 m3/s ----
Q_HIGH_THRESH_M3S = 100.0
Q_HIGH_MAX_MULT   = 5.0
Q_HIGH_POWER      = 1.0

# ---- Optional: small graph smoothness penalty (semi-supervised regularization) ----
USE_GRAPH_SMOOTHNESS = True
LAMBDA_SMOOTH        = 1e-3   # tune 1e-4 to 1e-2
...
# NEW: model filename to distinguish constrained-head version (recommended)
...
# Simple raw min check for targets (before any QC/scaling)
...
# ---------------------------------------------------------
# NEW: Q repair by COMID interpolation
#   - Q < 0 -> NaN -> interpolate within each COMID over time
# ---------------------------------------------------------
...
# -------------------------------------------------------------------
# OPTION 1 ONLY (training-time temp QC):
#   - mark implausible water temperatures as missing BEFORE masks/scaling
# -------------------------------------------------------------------
TEMP_QC_MIN_C = -5.0
TEMP_QC_MAX_C = 60.0
...
# Preserve availability masks BEFORE any sentinel replacement / scaling
nodes_df_raw["temp_avail_raw"] = ~nodes_df_raw[T_COL].isna()
nodes_df_raw["q_avail_raw"]    = ~nodes_df_raw[Q_COL].isna()

# -------------------------
# Optional row filter (recommended):
# Keep ALL rows where Q is observed (including Q == 0),
# and keep rows with missing Q only for higher stream orders (>= 5)
# -------------------------
STREAMORDER_MIN_FOR_Q_NA = 4

q_filter = pd.to_numeric(nodes_df_raw[Q_COL], errors="coerce").copy()
q_filter = q_filter.mask(q_filter == SENTINEL_MISS, np.nan)

so_filter = pd.to_numeric(nodes_df_raw["streamorde"], errors="coerce")

keep_rows = (
    q_filter.notna() |
    (q_filter.isna() & (so_filter >= STREAMORDER_MIN_FOR_Q_NA))
)
...
print("\n=== Applied early row filter (observed-Q retention + high stream-order NA retention) ===")
print(f"Rule: keep (Q observed) OR (Q is NA and streamorde >= {STREAMORDER_MIN_FOR_Q_NA})")
...
# IMPORTANT:
# - We do NOT overwrite Q==0 anymore.
# - Negative Q has been repaired earlier via interpolation (or left NaN if not repairable).
# - We will mask Q loss to only where q_avail_raw is True.
...
# For scaling: fill NaN with 0 only for scaler mechanics.
# Supervision is handled by masks (q_avail_raw, temp_avail_raw).
...
# -------------------------
# Dense tensors
# X: [T, N, F], Y: [T, N, 2], TempMask: [T, N], QMask: [T, N]
# -------------------------
...
# -------------------------
# Model: LSTM + 2×GAT, two-output head (Q, Temp) with sigmoid-constrained outputs
# -------------------------
class GraphLSTM_MTL(nn.Module):
    ...
    def forward(self, x_win, edge_index):
        ...
        yhat = torch.sigmoid(raw)                       # [N, 2] constrained to [0,1] in scaled space
        return yhat
...
# -------------------------
# Mixed loss: Q (masked NSE-like) + Temp (masked MSE)
#   - Q term computed ONLY on nodes where mq == True (observed Q)
#   - Temp term computed ONLY on nodes where mt == True (observed Temp)
#   - Optional high-flow weighting inside Q
#   - Optional graph smoothness penalty
# -------------------------
...
#   IMPORTANT: Q metrics computed ONLY where Q is observed (mask_q)
#              Temp metrics computed ONLY where Temp is observed (mask_t)
# -------------------------
...
# Diagnostics in scaled space (should now be bounded in [0,1])
...
# Safety rail: enforce nonnegative predicted Q after inverse transform
...
# node-level streamorder lookup aligned to node index
streamorder_by_node = (
...
# Physical plausibility diagnostics (test predictions)
...
#   NOTE: true_q_series is meaningful only where Q is observed.
# -------------------------
...
# build per-time masks for this COMID
...
# Observed vs Predicted scatter (Q observed-only)
...
# Observed vs Predicted scatter (Temp observed-only)
